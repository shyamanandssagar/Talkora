...
const LoginPage = () => {
  ...
  const handleLogin = (e) => {
    ...
    loginMutation(loginData, {
      onError: (err) => {
        // If backend says email not verified, redirect to verify page
        if (err?.response?.data?.redirectTo === "verify_email") {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-base-300 p-4" data-theme="night">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl border border-base-content/10">
        ...
        {/* LEFT PANEL */}
        <div className="w-full lg:w-1/2 bg-base-100 p-10 flex flex-col justify-between gap-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="form-control gap-1">
              <div className="flex items-center justify-between">
                ...
                <Link to="/forgot-password" className="text-xs text-primary hover:underline underline-offset-2 font-semibold">
                  Forgot password?
                </Link>
              </div>
              <label className="input input-bordered flex items-center gap-3 bg-base-200 border-base-content/10 focus-within:border-primary focus-within:bg-base-100 transition-all rounded-xl">
